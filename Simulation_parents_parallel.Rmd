...
G_func = function(filename, beta, MAF, n_sib = 0, N=1e5, M=1e5, block_size=1000){
  ...
  liabil = furture_lapply(1:iterations, function(i){
  ...
  #print(out)  #, future.seed = TRUE
  return(out)
  }) %>% 
   do.call(bind_cols, .) %>% 
   mutate(l_g_p1 = rowSums(select(., contains("p1"))), l_g_p2 = rowSums(select(., contains("p2")))) ##%>%
...
#plan("multisession")
k = G_func(filename = "test19", beta = beta_, MAF = MAF, n_sib = 2, N = 100, block_size = 10, M = 100)
k
```
