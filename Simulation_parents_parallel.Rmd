...
G_func = function(filename, beta, MAF, n_sib = 0, N=1e5, M=1e5, block_size=1000){
  ...
  l_g_sibs = do.call(cbind, l_g_sibs)
  col_names_g = get_names(c("l_g_partial"), id=FALSE, parents = FALSE, n_sib)
  colnames(l_g_sibs) = col_names_g
  

  out = bind_cols(tibble('l_g_partial_p1'=l_g_p1, 'l_g_partial_p2'=l_g_p2), as_tibble(l_g_sibs))
  return(out)
  },future.seed = TRUE) %>% 
...
#plan("multisession")
k = G_func(filename = "test5", beta = beta_, MAF = MAF, n_sib = 2, N = 100, block_size = 10, M = 100)
k
```
