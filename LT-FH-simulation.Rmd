...
for(i in 1:iterations){
  ...
  G1 = matrix(rbinom(block_size*N, 2, MAF), ncol=M, nrow=block_size, byrow = TRUE)
  G2 = matrix(rbinom(block_size*N, 2, MAF), ncol=M, nrow=block_size, byrow = TRUE)
  k = matrix(rnorm(block_size*M, 0, 0.0000001), ncol=M, nrow=block_size, byrow = TRUE)
  
  G[current_start:current_end,] = round((G1 + G2)/2 - k)
  
  l_g1[current_start:current_end] = c(sweep(sweep(G1[,], FUN = '-', STATS=2*MAF, MARGIN = 2), FUN='/',
                                           STATS=sqrt(2*MAF*(1-MAF)), MARGIN = 2)  %*% beta)
  l_g2[current_start:current_end] = c(sweep(sweep(G2[,], FUN = '-', STATS=2*MAF, MARGIN = 2), FUN='/',
                                           STATS=sqrt(2*MAF*(1-MAF)), MARGIN = 2)  %*% beta)
